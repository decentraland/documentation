...
In the snippet above, we use `engine.getEntityOrNullByName()` to fetch a reference to an entity named _Yellow Crate_. In the next line, we do `if (crate)` to ensure that there really is an entity by this name in the scene. If there isn't one, then the value of `crate` will be `null`. See [Combine with code]({{< ref "/content/creator/scene-editor/combine-with-code.md" >}}) for more info.
...
{{< hint info >}}
**💡 Tip**: You can also fetch [Smart Items]({{< ref "/content/creator/scene-editor/smart-items/smart-items.md" >}}) from your scene in the same way, and do whatever you want with them via code. See [Combine with code]({{< ref "/content/creator/scene-editor/combine-with-code.md" >}}) for more info.
{{< /hint >}}
