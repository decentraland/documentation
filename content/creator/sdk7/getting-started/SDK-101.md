...
{{< hint warning >}}
**📔 Note**: You may notice that now the avocado has shrinks to a size of 0, but still exists. We won't cover this in this first tutorial, but in a real scenario you should make sure you delete the entity after the tween is over, to optimize your scene's performance. See [On tween finished][here]({{< ref "/content/creator/sdk7/3d-essentials/move-entities.md#on-tween-finished" >}}).
{{< /hint >}}

## Reference an item from the editor

Your code can also modify any items that you added visually by dragging and dropping using the Scene Editor. To fetch these items, we can use the function `engine.getEntityOrNullByName()`, and pass the name that's assigned to the item on the entity tree of the editor.
