...
Before you deploy your scene to Decentraland, make sure the scene runs well in preview using the latest version of the Decentraland SDK. See [debug in preview]({{< ref "/content/creator/deprecated/scenes/debugging/debug-in-preview.md" >}}).
...
See [The test server]({{< ref "/content/creator/deprecated/scenes/publishing/publishing.md#the-test-server" >}}) for details.
...
This will enable the option of opening a panel that displays stats that update in real time, including material count, entity count, processed messages, etc. See [view scene stats]({{< ref "/content/creator/deprecated/scenes/debugging/debug-in-preview.md#view-scene-stats" >}}) for details.
